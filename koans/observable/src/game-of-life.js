...
SAMURAIPRINCIPLE.GameOfLife = function() {
    ...
    this.toggleCellState = function(row, column) {
        ...
        this.dispatchEvent('cellStateChanged', row, column, isAlive[key] || false);
		//return [row, column, isAlive[key]];        
    };
    this.tick = function() {
        var key, parts, row, column, numberOfNeighbours = {},
            neighbourKey;
        for (key in isAlive) {
            parts = key.split('_');
...
jQuery.fn.extend({
    gameOfLifeWidget: function(gameOfLife, rows, columns, animationDuration){
        return this.each(function() {
            ...
            rootElement.find('td').on('click', function(){
            	gameOfLife.toggleCellState(3,4, {});
            });
            gameOfLife.addEventListener('cellStateChanged', function(row, col, isAlive){
            	rootElement.find('.grid tr:nth-child(3) td:nth-child(4)').toggleClass('alive');
            });
        });
    }
});